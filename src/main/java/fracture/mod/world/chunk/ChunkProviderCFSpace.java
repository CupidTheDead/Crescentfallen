...
public abstract class ChunkProviderCFSpace extends ChunkProviderCommon {

	// note, this may not be called anywhere.
	// this is a custom chunk provider for a space dimension or planet.
	//

	// It defines custom planetary terrain generation in a space dimension,
	// including:

	// Procedural terrain height using Perlin noise.

	// Crater generation for alien aesthetics.

	// Biome-based surface layer replacement.

	// Supports structure generation and decoration.

	// Fully customizable via abstract methods for different planets.
	...
	public ChunkProviderCFSpace(World par1World, long seed, boolean mapFeaturesEnabled) {
	...
	public void generateTerrain(int chunkX, int chunkZ, ChunkPrimer primer) {
		...
		for (int x = 0; x < ChunkProviderCFSpace.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderCFSpace.CHUNK_SIZE_Z; z++) {
				...
				final double featureFilter = this.noiseGen5.getNoise(chunkX * 16 + x, chunkZ * 16 + z)
						* ChunkProviderCFSpace.MAIN_FEATURE_FILTER_MOD;
				final double largeFilter = this.noiseGen6.getNoise(chunkX * 16 + x, chunkZ * 16 + z)
						* ChunkProviderCFSpace.LARGE_FEATURE_FILTER_MOD;
				final double smallFilter = this.noiseGen7.getNoise(chunkX * 16 + x, chunkZ * 16 + z)
						* ChunkProviderCFSpace.SMALL_FEATURE_FILTER_MOD - 0.5;
				mountainHeight = this.lerp(smallHillHeight, mountainHeight * this.MOUNTAIN_HEIGHT_MOD,
						this.fade(this.clamp(mountainHeight * 2, 0, 1)));
				valleyHeight = this.lerp(smallHillHeight,
				...
				for (int y = 0; y < ChunkProviderCFSpace.CHUNK_SIZE_Y; y++) {
	...
	public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn) {
		...
		for (int var8 = 0; var8 < 16; ++var8) {
			for (int var9 = 0; var9 < 16; ++var9) {
				final int var12 = (int) (this.noiseGen4.getNoise(x * 16 + var8, z * 16 + var9) / 3.0D + 3.0D
						+ this.rand.nextDouble() * 0.25D);
				int var13 = -1;
				Block var14 = this.getGrassBlock();
				Block var15 = this.getDirtBlock();

				for (int var16 = ChunkProviderCFSpace.CHUNK_SIZE_Y - 1; var16 >= 0; --var16) {
	...
	public Chunk generateChunk(int x, int z) {
		...
		try {
			...
			this.biomesForGeneration = this.world.getBiomeProvider().getBiomes(this.biomesForGeneration, x * 16, z * 16,
					16, 16);
			...
			GCLog.severe("Error caught in planetary worldgen at coords " + x + "," + z
					+ ".  If the next 2 lines are showing an Add-On mod name, please report to that mod's author!");
			e.printStackTrace();
		}
	...
	public void createCraters(int chunkX, int chunkZ, ChunkPrimer primer) {
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++) {
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++) {
				for (int x = 0; x < ChunkProviderCFSpace.CHUNK_SIZE_X; x++) {
					for (int z = 0; z < ChunkProviderCFSpace.CHUNK_SIZE_Z; z++) {
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen5
								.getNoise(cx * 16 + x, cz * 16 + z) / this.CRATER_PROB) {
