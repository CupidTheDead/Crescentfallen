...
public class MapGenAddonCaveGen extends MapGenBaseMeta {
	...
	private void addRoom(long seed, int originalX, int originalZ, ChunkPrimer primer, double x, double y, double z) {
		this.addTunnel(seed, originalX, originalZ, primer, x, y, z, 1.0F + this.rand.nextFloat() * 6.0F, 0.0F, 0.0F, -1,
				-1, 0.5D);
	}

	private void addTunnel(long seed, int originalX, int originalZ, ChunkPrimer primer, double x, double y, double z,
			float p_180702_12_, float p_180702_13_, float p_180702_14_, int p_180702_15_, int p_180702_16_,
			double p_180702_17_) {
		...
		for (boolean flag = rand.nextInt(6) == 0; p_180702_15_ < p_180702_16_; ++p_180702_15_) {
			...
			if (!flag2 && p_180702_15_ == j && p_180702_12_ > 1.0F && p_180702_16_ > 0) {
				this.addTunnel(rand.nextLong(), originalX, originalZ, primer, x, y, z, rand.nextFloat() * 0.5F + 0.5F,
			...
			if (flag2 || rand.nextInt(4) != 0) {
				...
				if (x >= d0 - 16.0D - d2 * 2.0D && z >= d1 - 16.0D - d2 * 2.0D && x <= d0 + 16.0D + d2 * 2.0D
						&& z <= d1 + 16.0D + d2 * 2.0D) {
									...
									for (int j2 = l; j2 > l2; --j2) {
										...
										if (d9 > -0.7D && d10 * d10 + d9 * d9 + d8 * d8 < 1.0D) {
											...
											IBlockState iblockstate2 = MoreObjects.firstNonNull(
													primer.getBlockState(j3, j2 + 1, i2), Blocks.AIR.getDefaultState());

											if (this.isTopBlock(primer, j3, j2, i2, originalX, originalZ)) {
												flag1 = true;
											}
											this.digBlock(primer, j3, j2, i2, originalX, originalZ, flag1, iblockstate1,
													iblockstate2);
										}
									}
								}
