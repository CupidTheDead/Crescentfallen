		...
		for (final boolean var28 = var25.nextInt(6) == 0; par15 < par16; ++par15) {
			...
			if (var54 || var25.nextInt(4) != 0) {
				...
				if (xPos >= var19 - 16.0D - caveWidth * 2.0D && zPos >= var21 - 16.0D - caveWidth * 2.0D
						&& xPos <= var19 + 16.0D + caveWidth * 2.0D && zPos <= var21 + 16.0D + caveWidth * 2.0D) // CHECKED
				{
								...
								for (int var50 = caveMaxY - 1; var50 >= caveMinY; --var50) {
									...
									if (var59 * var59 + var51 * var51 + var46 * var46 < 1.0D) {
										if (var51 > -0.7D) {
											...
											if (block == MarsBlocks.marsBlock || block == MarsBlocks.blockSludge
													|| block == MarsBlocks.vine) {
											...
											if (Blocks.AIR == block) {
												if (blockAbove == MarsBlocks.marsBlock && this.rand.nextInt(200) == 0) {
													...
													while (Blocks.AIR == block && var50 + modifier > 0) {
														primer.setBlockState(var42, var50 + modifier, var45,
																MarsBlocks.vine
																		.getStateFromMeta(Math.abs(modifier) % 3));
														modifier--;
														block = primer.getBlockState(var42, var50 + modifier, var45)
																.getBlock();
													}
												} else if (blockBelow == MarsBlocks.marsBlock
														&& this.rand.nextInt(200) == 0) {
													primer.setBlockState(var42, var50, var45,
															MarsBlocks.blockSludge.getStateFromMeta(0));
												}
											}
										}
									}
